  ...
  for (s in seq_along(subgroups)) {
    ...
    for (i in seq_len(num_outcomes)) {
      ...
      for (j in seq_len(num_combos)) {
        ...
        Xvar_sg  <- if (!is.null(controls)) {
          # Use quadvariance (n denominator)
          n_sg <- nrow(controls)
          if (n_sg > 1) stats::var(controls) * (n_sg - 1) / n_sg else 0
        } else 0
  ...
  for (b in seq_len(B)) {
    ...
    for (i in seq_len(num_outcomes)) {
      for (s in seq_along(subgroups)) {
        ...
        Y_sg  <- Y_boot[idx_sg, i, drop = FALSE]
        ...
        for (j in seq_len(num_combos)) {
          ...
          # Compute control variates as in the Stata implementation
          controls_b <- if (ncol(cur_X) > 1) cur_X[,-1,drop=FALSE] else NULL
          Xbar_b     <- if (!is.null(controls_b)) colMeans(controls_b) else 0
          Xvar_b     <- if (!is.null(controls_b)) {
            quadvariance(controls_b)
          } else 0

          est_b <- runreg(cur_X, cur_y, Xbar_b, Xvar_b,
                          mean(treat==1), mean(treat==0), 1)
