...
fh_abregact = fopen("abregact.csv", "w")
fh_abregboot = fopen("abregboot.csv", "w")
fh_pact = fopen("pact.csv", "w")
fh_pboot = fopen("pboot.csv", "w")
// Write headers
fput(fh_abregact, "i,j,k,value")
fput(fh_abregboot, "l,i,j,k,value")
fput(fh_pact, "i,j,k,value")
fput(fh_pboot, "l,i,j,k,value")
...
// 			pi_s = pi_prob[l,1]
			...
			// Add print statement for abregact here
            val_abregact = get(abregact, (i,j,l))
            printf("abregact[%g,%g,%g] = %g\n", i, j, l, val_abregact)
			// Save to CSV
			fput(fh_abregact, sprintf("%g,%g,%g,%g", i, j, l, get(abregact, (i,j,l))))
...
// 				get(regact, (j,k,l))
				...
				// BIGG
				// Save to CSV
				// After calculating abregboot in the bootstrap loop
				// Add this inside the bootstrap loop after the abregboot calculation
				if (i <= 10) {  // Only print first 10 bootstrap samples
					val = get(abregboot, (i,j,k,l))
					printf("abregboot[%g,%g,%g,%g] = %g\n", i, j, k, l, val)
				}
				// Save to CSV
                fput(fh_abregboot, sprintf("%g,%g,%g,%g,%g", i, j, k, l, val))
			}
        }
    }
}
...
// 			printf("pval:%f",p)
			...
			printf("pact[%g,%g,%g] = %g\n", i, j, k, p)
			// Save pact to CSV
            fput(fh_pact, sprintf("%g,%g,%g,%g", i, j, k, p))
			for (l=1; l<=B; l++)
			{
				sp = 1 - (sum(get(abregboot, (.,i,j,k)) :>= get(abregboot, (l,i,j,k)) * J(B,1,1))) / B;
				put(sp, pboot, (l,i,j,k))
				// Save pboot to CSV
                fput(fh_pboot, sprintf("%g,%g,%g,%g,%g", l, i, j, k, sp))
				if (l <= 10) { // Only print first 10 bootstrap samples
                    printf("pboot[%g,%g,%g,%g] = %g\n", l, i, j, k, sp)
